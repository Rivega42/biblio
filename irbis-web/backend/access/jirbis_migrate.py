...
# --------------------------------------------------------------------------- #
# mysqldump -> строки (turnkey узла 4): parse_dump(open('jirbis2.sql').read())
# -> plan(...). Колонки берём из CREATE TABLE (mysqldump-дефолт без списка колонок
# в INSERT) либо из явного списка INSERT. Кодировка дампа — utf8 (по RESPONSE).
# Ограничение слайса: рассчитан на стандартный формат mysqldump; экзотика (`;`
# внутри строкового литерала) — вне слайса, на реальном дампе при необходимости
# усилить. На синтетике (test) проверены кириллица/escape/NULL/без-списка-колонок.
# --------------------------------------------------------------------------- #
_ESC = {'n': '\n', 't': '\t', 'r': '\r', '0': '\0', 'b': '\b'}


def _columns_from_create(sql, table):
    """Порядок колонок из ``CREATE TABLE`` (для INSERT без списка колонок)."""
    m = re.search(r'CREATE TABLE\s+[`"]?' + re.escape(table)
                  + r'[`"]?\s*\((.*?)\)\s*(?:ENGINE|;)', sql, re.S | re.I)
    if not m:
        return []
    cols = []
    for line in m.group(1).splitlines():
        line = line.strip()
        cm = re.match(r'[`"]([A-Za-z0-9_]+)[`"]\s+\S', line)
        head = line.split(None, 1)[0].upper().strip('`"') if line else ''
        if cm and head not in ('PRIMARY', 'KEY', 'UNIQUE', 'INDEX', 'CONSTRAINT',
                               'FULLTEXT', 'FOREIGN'):
...
def _split_tuples(s):
    """Разбить ``(...),(...)`` на список «сырых» тел кортежей (с учётом кавычек)."""
    out, i, n = [], 0, len(s)
    while i < n:
        if s[i] == '(':
            j, depth, inq = i + 1, 1, False
            while j < n and depth:
                c = s[j]
                if inq:
                    if c == '\\':
...
def _parse_row(tup):
    """Разобрать тело кортежа ``1,'a','b\\'c',NULL`` -> list значений (NULL->None)."""
    res, i, n = [], 0, len(tup)
    while i < n:
        while i < n and tup[i] in ' \t':
            i += 1
        if i >= n:
            break
        if tup[i] == "'":
            ...
            while i < n:
                c = tup[i]
                if c == '\\' and i + 1 < n:
                    buf.append(_ESC.get(tup[i + 1], tup[i + 1]))
                    i += 2
                    continue
                if c == "'":
            ...
            while j < n and tup[j] != ',':
                j += 1
            tok = tup[i:j].strip()
            res.append(None if tok.upper() == 'NULL' else tok)
            i = j
        while i < n and tup[i] in ' \t':
            i += 1
        if i < n and tup[i] == ',':
...
def parse_table(sql_text, table):
    """Строки таблицы из mysqldump-текста -> ``list[dict]`` (колонка->значение)."""
    create_cols = _columns_from_create(sql_text, table)
    rows = []
    pat = (r'INSERT INTO\s+[`"]?' + re.escape(table)
           + r'[`"]?\s*(?:\(([^)]*)\))?\s*VALUES\s*(.*?);')
    for m in re.finditer(pat, sql_text, re.S | re.I):
        cols = ([c.strip().strip('`"') for c in m.group(1).split(',')]
                if m.group(1) else create_cols)
        if not cols:
            continue
        for tup in _split_tuples(m.group(2)):
            rows.append(dict(zip(cols, _parse_row(tup))))
    return rows


def parse_dump(sql_text, tables=None):
    """mysqldump-текст -> ``{table: [row, …]}`` для целевых таблиц (migrate+cms).

    Затем: ``plan(parse_dump(open('jirbis2.sql').read()))`` — узел 4 одной строкой."""
    tables = tables or (list(MIGRATE_TABLES) + list(CMS_TABLES))
    out = {}
    for t in tables:
        if t in sql_text:
            rows = parse_table(sql_text, t)
            if rows:
                out[t] = rows
    return out
