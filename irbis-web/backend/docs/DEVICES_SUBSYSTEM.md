...
- Источник истины карт в ИРБИС — RDR 30/24/28; здесь СВОЙ реестр привязок (живой
  ИРБИС не пишем, #222). `rdr_sync_plan(ticket)` — DRY-RUN план правок RDR-полей
  30/24/28 (для явной админ-синхронизации/экспорта), сам ИРБИС не пишет.
...
- Сверка с каталогом — сид `catalog` (`_InventoryCatalogAdapter` в core →
  `catalog.exemplars_at(db,location)` = экземпляры с 910^D==место + `find_exemplar`
  для item_known). report даёт present/missing/foreign/unknown по реальному ЭК.
...
- **IAbis (роль A), реальная книговыдача** через `circulation`-сид (`_DeviceCircAdapter`):
  ...
  (mfn+заглавие по штрихкоду), `SetBookState` (910^A=enum), `GetBookCenz` (возрастной
  ценз: 900^Z без '+', иначе 18). Decision → `{Success, Reasons, Due}`.
...
py -3.12 tests/test_compat_devices.py  # 88 — шим (IAbis+tag+Фаза2+SIP2+ценз)
py -3.12 tests/test_device_routes.py   # 23 — HTTP /api/devices/*
py -3.12 tests/test_station_integration.py  # 15 — e2e (выдача + сверка инвент. + ценз)
```
Всего 453, in-memory, без DB-сервера (`test_readers.py` — 19 с rdr_sync_plan). Стиль `test_acquisition.py`/`test_circ_routes.py`
...
- Готово: домен `devices`, `locker` (SafeKeeper→holds), compat-шим
  ...
  **сверка ТСД-инвентаризации с каталогом** (910^D), **GetBookCenz** (возрастной ценз),
  **rdr_sync_plan** (DRY-RUN экспорт карт в RDR).
- Дальше (вне кода/деплой): реальный TCP-транспорт SIP2/считывателя на клиенте
  станции; явная запись карт в прод-RDR по плану; booking-by-face flow.
- Заблокировано: Фаза 3 (привод реальных станций) — нужен station-facing трейс/билд.
