...
py -3.12 tests/test_compat_devices.py  # 79 — шим (станц. + IAbis + tag + Фаза 2)
py -3.12 tests/test_device_routes.py   # 23 — HTTP /api/devices/* (e2e + Фаза 2)
```
Всего 330, in-memory, без DB-сервера. Стиль `test_acquisition.py`/`test_circ_routes.py`
...
- Готово: домен `devices`, `locker` (SafeKeeper→holds), compat-шим
  ...
  **Фаза 2** — периметр (ворота/счётчик/полки/СКУД ingestion), **ТСД-инвентаризация**
  (`inventory`), **камеры/FaceID** (`vision`, токены без биометрии).
- Дальше: остаток IAbis (SetBookState/GetUserDebts/GetBookCenz), запись карт
  обратно в прод-RDR (30/24/28), реальный транспорт считывателя (сокет к железу на
  клиенте станции), адаптер сверки инвентаризации с каталогом («место→экземпляры»).
