...
> Снято из бинарей `C:\IRBIS64\irbis64_client.dll` (439 КБ) и `IRBIS64.dll` (975 КБ) через `pefile` (см. `irbis-web/backend/re_dll.py`). Обе — **32-битные Delphi** (секции CODE/DATA/BSS, импорт RTL). Это **таблицы экспортов = канонический API** (имена функций). Тела ключевых функций движка **уже декомпилированы** Ghidra 12.1.2 (headless) — псевдокод в [`IRBIS64_engine_decompiled.c`](IRBIS64_engine_decompiled.c) (7 функций: `Irbis_Format`/`Irbis_InitPFT`/`IrbisFindPosting`/`Irbisfind`/`IrbisSearch_Range`/`InsertTerm`/`UMARCI`) и [`IRBIS64_UNIFOR_decompiled.c`](IRBIS64_UNIFOR_decompiled.c) (диспетчер `UNIFOR`), а их прозаический разбор (алгоритм поиска/инверт.-файла, конвейер формата) — в [DECOMPILATION_FINDINGS](DECOMPILATION_FINDINGS.md). Заполняет «DLL-внутреннюю логику», которой нет в INI. Дополняет [PROTOCOL_REFERENCE](./).
...
## Тела функций (Ghidra-декомпиляция выполнена)
Алгоритмы ядра **разобраны** (Ghidra 12.1.2 headless) — полный анализ в [DECOMPILATION_FINDINGS](DECOMPILATION_FINDINGS.md):
- **Поиск/инвертированный файл** — `IrbisFindPosting`+`Irbisfind`+`IrbisSearch_Range`+`InsertTerm`: дерево узлов (`.l01`/`.n01`) → цепочка 20-байт блоков постингов (`.ifp`), seek/read по цепочке, fast-path `-1001` (постинг в узле), big-endian числа, постинг = 32-байт слот, B-tree split при вставке, булев поиск = операции над множествами MFN.
- **Формат-движок A1** — `Irbis_Format`+`Irbis_InitPFT`+`UNIFOR`: guard MFN (`-102`) → init-контекст → парс-цикл → пост-обработка; `UNIFOR` — строковый диспетчер по первому символу команды (`*arg-'!'`) с вложенными под-switch, результат аппендится в выходной буфер.
- **`UMARCI`** — примитив импорта UNIMARC (диспетч по цифре `0..5`).

Остаточные пробелы (нужен прогон по конкретным внутренним `FUN_*`): точная семантика шести веток `UMARCI`, полная карта верхнего switch `UNIFOR` (восстановлены `!`/`#`/`$`/`+`/`2`/`3`), точный формат записи постинга по `AType` команды `IC_getposting`. Парсинг `.gbl` (глобальная корректировка) пока не декомпилирован.
