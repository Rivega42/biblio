...
## 7. Проводной формат TCP-пакета — ✅ СНЯТ (Проход Б)
> Этот gap **закрыт эмпирически**: байтовый формат, модель соединения (одно TCP на запрос), кадры запрос/ответ, кодировки и числовые коды (`0`/`-3337`/`-3338`) сняты с живого сервера `:6666` зондом `tools/irbis-probe/probe.py`. См. **[WIRE_PROTOCOL.md](WIRE_PROTOCOL.md)**. Ниже — исходный список того, что требовалось снять (выполнено, кроме мелких ПБ-1..4 в WIRE_PROTOCOL §10).

Документ-первоисточник — только C-API DLL; байтовый формат в нём отсутствовал, поэтому снимали с сервера: `TODO(Проход Б #12: снять реальный проводной формат с живого сервера/клиента BookCabinet)`:
