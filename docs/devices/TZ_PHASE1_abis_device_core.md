...
- **item** (экземпляр) — `id, catalog_record_id, inv_number, barcode(910^B), status(910^A → enum BookState), location(910^D), shelf_mark(903), issue_count(999)`.
  - ⚠️ **Групповые экземпляры:** статус `910^A` может быть буквенным `U`/`C` — это группа экземпляров с подполями `1`=всего и `2`=выдано (доступность = выдано<всего → AllAvailable, иначе OnHands; [EASYBOOKABIS_IRBIS_MAP.md](EASYBOOKABIS_IRBIS_MAP.md) §4). Для режима 2 добавить поля `group_total`/`group_issued` (или отдельную сущность), иначе сломается учёт группового фонда.
...
**Что даёт бесшовность:** мы полностью задокументировали device‑facing контракт (`/easybookdll/*`, маршруты `api.svc`, DTO) — поэтому Biblio может предъявить **контрактно‑совместимый** API (по сути 1:1; известные баги-расхождения `EntranceModify`/`DesktopDeviceConfigGet` копировать не нужно — на подхват не влияют). Тогда «подхват» = смена адреса в конфиге устройства.

**\* Важный нюанс (device‑facing Basic):** агент EasyBook ходит на `/easybookdll/*` с **захардкоженными в resx** кредами `ServiceLogin:‹…›` (см. [TAGSERVICE_FUNCTION_MAP.md](TAGSERVICE_FUNCTION_MAP.md) §6) — override логина в `cfg3` нет. Поэтому для **бесшовного** подхвата существующего TagService **наш `/easybookdll/*` обязан принимать именно этот унаследованный Basic** (whitelist legacy-креды), иначе будет 401. Альтернатива без бесшовности — поставить наш Reader Agent. Зафиксировано в критериях §8.

**Где не бесшовно** — там, где адрес зашит в прошивку станции/локера; это единственный реальный барьер, и он только на станционном железе (не на настольных/воротах/камерах/ТСД).

**Вывод:** настольные считыватели (с учётом legacy-Basic), ворота, камеры, ТСД — **бесшовно** (конфиг). Станции/постаматы/полки — бесшовно **при условии** конфигурируемого URL прошивки; иначе нужен доступ к прошивке (Фаза 3).
...
| **Аудит действий** | LAS `ExternalLog` (enum `ExternalLogTypes`: BookCheckOut/In, OrderAdded, ReaderRFIDAdded, UserEdited, … 32 типа) | таблица `audit_log` (§3) + `ExternalLogAdd/Get`; каждая операция логируется |
...
**Аудиты** — особо: модель аудита уже извлечена полностью — это перечисление `ExternalLogTypes` (32 типа событий: книговыдача/возврат, заказы, привязка карт, действия пользователей и т.д.) + таблица `audit_log`. То есть аудит‑трейл воспроизводится 1:1.
...
- Книговыдача/возврат/продление и формуляр работают через настольный считыватель **в обоих режимах** (`IrbisBackend` к реальному ИРБИС; `BiblioNativeBackend` к своей БД) — с идентичным поведением, включая **групповые экземпляры** (`910^A`=U/C, всего/выдано).
- Регистрация читателя (ЧБ и ЕКП) и привязка карт/фото — через ABIS‑порт (поля 28/30/24).
- **Логин по карте с PIN** (поле 103) и **политика прав/перерегистрации/долгов/возраста** работают в обоих режимах (в режиме 2 — нативный эквивалент PFT `ReaderRightsPft`/`ReturnRightsPft`/`@cenz`, т.к. PFT там нет).
- Существующий **EasyBook TagService подхватывается** сменой `ServiceURL`/`[IRBIS]` — **при условии**, что `/easybookdll/*` Biblio принимает унаследованный захардкоженный Basic (`ServiceLogin:‹…›`); иначе — установка нашего Reader Agent.
- Аудит‑лог фиксирует операции (типы `ExternalLogTypes`, 32 шт.).
- Миграция: вычитка **`RDR` и каталога** из ИРБИС в нативную БД, расхождений нет (сверка по выборке). **`RQST` (заказы) — после уточнения раскладки полей** (поле 691/RQST — инференс, [OPEN_QUESTIONS.md](OPEN_QUESTIONS.md) §6); до этого заказы/бронь ведутся в Device Service.
