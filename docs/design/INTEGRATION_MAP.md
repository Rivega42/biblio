...
| # | Источник (поле/запись) → цель | Триггер / семантика | Дизайн? (§) | Код? (модуль / «нет») | Статус | Примечание |
|---|---|---|---|---|---|---|
| 8.1 | IBIS поле **902** (сигла) → **SK** (сводный каталог) | 902^s = сигла держателя; SK ссылается на записи библиотек-участниц (`&uf('DSK,!I=',…)`) | CAPABILITY_MAP §4/§11, DB_CATALOG_VARIANTS | 🟡 слайс `access/union.py` (`UnionCatalog.ingest` свод по сигле 902^s в `union_holding` под сводной записью; федеративный `search`); тест `test_union.py` 32/0; **роут + ingest из реального каталога pending** | 🟡 | Домен-модуль есть+тест; в продукт не разведён. Δ2026-06-27. |
| 8.2 | SK 907^Z дедуп ↔ записи-участницы | Ключ дедупликации (шифр/ISBN) при импорте в сводный | DB_CATALOG_VARIANTS | 🟡 слайс `union.dedup_key` (приоритет ISBN>шифр(903)>title\|year), идемпотентный `ingest` сводит дубли в одну запись; тест `test_union.py` | 🟡 | Дедуп есть в модуле; импорт-конвейер pending. Δ2026-06-27. |
...
| # | Источник (поле/запись) → цель | Триггер / семантика | Дизайн? (§) | Код? (модуль / «нет») | Статус | Примечание |
|---|---|---|---|---|---|---|
| 10.1 | RDR поле **140** (`IRI=`) → **ZAPR/IRI** постоянные запросы | Профиль ИРИ; ZAPR хранит запрос (`2^B`ПТ-часть, `2^C`библио-часть), переигрывается по расписанию против IBIS | FINDINGS_09, DB_SERVICE, SPEC_reader_jirbis | 🟡 слайс `access/sdi.py` (`SdiService`: профили постоянных запросов + `run_profile` переигрывает против каталога-хендла, отдаёт НОВЫЕ попадания с прошлого прогона, идемпотентно); тест `test_sdi.py` 29/0; **роут + привязка к RDR.140 + планировщик pending** | 🟡 | SDI-домен есть+тест; в продукт не разведён. Δ2026-06-27. |
...
| 11.3 | circulation → **NotificationQueue** (A6) | События выдачи (`hold_ready/fine_charged/renewal_confirmed/lost_confirmed/staff_alert/fine_paid`) | SPEC_engine_notifications, ws3 §8 | 🟡 `circulation._emit` вызывает `notifier.enqueue` (enqueue замкнут); шаблоны EventCatalog полны (commit `e0818d3`); читатель ВИДИТ нотисы в портальном инбоксе (`core.notifications_inbox` — этот канал доставки разведён). Диспетч во ВНЕШНИЕ каналы (email/SMS) — НЕ начат (активного агента нет). | 🟡 | Эмиссия + шаблоны + портальный inbox-канал замкнуты; внешняя доставка (email/SMS) не разведена — пометить ✅ после неё. |
...
|---|---|---|---|
| ✅ разведено (код+тесты) | ~39 | ~8 | **~47 (~75%)** |
