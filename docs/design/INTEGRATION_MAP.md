...
|---|---|---|
| ✅ preserved (дизайн + код) | **22** | ~37% |
| 🟡 designed-not-wired | **6** | ~10% |
| ❌ at-risk-of-loss | **31** | ~53% |
| **Итого рёбер** | **59** | 100% |

> Примечание о счёте: фактический пересчёт строк таблицы даёт **59** рёбер (кластеры 1–11, +1.11 acq→circ).

✅ (22): **базовые** — 1.11 поступление→выдаётся, 2.1/2.2 `910^A` flip, 5.2 →КСУ выбытия, 6.1 `^3`-on-save, 3.2 долг/лимиты, 3.4 категория→политика, 11.5 ФЛК-дублетность. **Батч этой сессии (HEAD `b05c23b` + `fa66a09`):** 11.3 диспетч уведомлений в каналы · 1.1–1.5 ToCat (реформат БО→каталог, поле-66, 910/938/901, VD=DEL) · 2.4/2.5 RDR.40-материализация поля 40 + архив RDR_ARH · 4.1/4.4/4.5 ККО-агрегат экз×студенты + Move691 + архив 692 · 6.2/6.3/6.4 autoin + индекс `AR=` над `^3` + ФЛК `^3`.

> Вывод (обновлён 2026-06-22): **крупный батч связности замкнут — рёбер ✅ выросло с 8 до 22 (~14%→~37%).** Прибавлены: диспетч уведомлений (11.3), ToCat комплектование→каталог (1.1–1.5), материализация RDR.40 + архив (2.4/2.5), полный ККО + Move691 + архив 692 (4.1/4.4/4.5), швы авторитетов autoin/`AR=`-индекс/ФЛК-`^3` (6.2/6.3/6.4) — всё через опц. handle'ы, back-compat. **Честная оговорка о глубине:** этот батч восстановлен из работы агентов, остановленных на шаге коммита (нестабильность среды); каждое ребро покрыто тестами и общая сьюта зелёная (**2232/0**, регрессия чиста), но это test-pass-верификация, без построчного ревью каждой реализации — глубину (особенно ToCat-FST-маппинг «ключевые поля, остальное TODO») предстоит до-ревьюить. **Остаётся 🟡:** полнота ToCat-маппинга, частичные хвосты ККО/авторитетов. **Остаётся ❌:** Move691/связка VUZ полностью (4.2/4.3), КСУ-поступления↔экз (5.1/5.3), ПТ↔права (кластер 7), сводный каталог (кластер 8), поиск-по-связи (9.x), ИРИ/SDI (10.x).
