...
**Разводимы сейчас (без новых модулей):** 12.6/12.7 (врезать `record_event` в circulation.checkout/holds.place_hold) · 11.3 нотиф-dispatch · 11.2 gbl-исполнение (`supported=False`) · 3.3 утеря→PAY (цена из 910^E + проводка) · 11.4 seed_vocab→circulation (вместо хардкода `_LIMIT_MATRIX`).
