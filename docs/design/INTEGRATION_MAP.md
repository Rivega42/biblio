...
|---|---|---|
| ✅ preserved (дизайн + код) | **8** | ~14% |
| 🟡 designed-not-wired | **13** | ~22% |
| ❌ at-risk-of-loss | **38** | ~64% |
| **Итого рёбер** | **59** | 100% |

> Примечание о счёте: фактический пересчёт строк таблицы даёт **59** рёбер (кластеры 1–11, +1.11 acq→circ); прежний заголовок «44» был занижен. Текущие итоги выше получены прямым подсчётом статусов.

✅ (8): 1.11 Комплектование→Книговыдача (поступление→выдаётся, commit `0ec7335`) · 2.1 / 2.2 Catalog↔Circulation `910^A` flip (**первое подлинно межмодульное ребро**, commit `cf97183`) · 5.2 Книговыдача→КСУ выбытия (commit `0ec7335`) · 6.1 Authority↔Catalog `^3`-on-save (commit `cf97183`) · 3.2 долг/лимиты · 3.4 категория→политика · 11.5 ФЛК-дублетность→индекс. Из них **1.11/2.1/2.2/5.2/6.1 — реально межмодульные** (acquisition↔circulation↔catalog↔authority); 3.2/3.4/11.5 — внутримодульные. Частично замкнуто (🟡): **4.4** КО↔выдача (Кко учитывает выданные/на руках, commit `0ec7335`).

> Вывод (обновлён): **межмодульная ткань продолжает замыкаться.** К ранее разведённым Catalog↔Circulation (`910^A`, 2.1/2.2) и Authority↔Catalog (`^3`, 6.1, commit `cf97183`) добавлены **3 ребра комплектования/КО↔выдача** (commit `0ec7335`): поступление→экземпляр выдаётся (1.11), Кко учитывает выданные (4.4), списание→КСУ выбытия (5.2) — через опциональные handle'ы, back-compat. Модули `acquisition.py`/`bookprovision.py` теперь существуют и связаны с `circulation`. Кросс-БД `.gbl` (NEWMFN/NEWREC/CORREC) **исполняется** (commit `cbde540`, 11.2). **Остаётся 🟡:** диспетч уведомлений A6 (11.3), полный агрегат ККО 693×студенты (4.4), индексация `^3`/606/607 (6.4), autoin-хук (6.2), ФЛК-проверка `^3` (6.3), RDR.40-материализация/архив (2.4/2.5). **Остаётся ❌:** ToCat-реформат БО→каталог (1.1–1.5), Move691/связка VUZ (4.1–4.3), КСУ-поступления↔экз (5.1/5.3), ПТ↔права (кластер 7), сводный каталог (кластер 8).
