...
| 1.3 | CMPL → IBIS поле **938** | Связь перенесённой записи с заказом подписки (`pkj.gbl`) | ws2 §8, FIELD_DICTIONARY (938→CMPL период) | ✅ `to_catalog._carry_938` переносит 938 (период подписки) CMPL→IBIS (acquisition.py:1307) | **✅** | Связь каталог↔подписка; тест `test_tocat::to_catalog_full_checks` [1.3]. **Реклассификация 2026-06-27 (карта отставала).** |
| 1.4 | CMPL экз → IBIS поле **901** (техн. путь) | При переносе экз. проставляется технологический путь | DB_ACQUISITION A.3.1/A.8 | ✅ `to_catalog._carry_901` строит 901 (^B №экз + пункты ТП) на экземпляр (acquisition.py:1312) | **✅** | 901 техпуть; тест `test_tocat` [1.4] (901 на экз. + явный 901). **Реклассификация 2026-06-27.** |
| 1.5 | ToCat → исходник CMPL: `VD=DEL` | FST-строка поля 66 ставит пометку на удаление исходной БО | ws2 §8 (AC3), DB_ACQUISITION A.8 | ✅ `to_catalog._mark_source_deleted` ставит `VD=DEL` (66^a='DEL') на исходник после переноса; идемпотентно (`already_transferred`) | **✅** | Анти-дубль; тест `test_tocat::trigger66_checks` (`is_source_deleted`). **Реклассификация 2026-06-27.** |
...
| 4.2 | VUZ контингент → RDR поля **90/69** | **LinkVuz.gbl**: студент↔дисциплины (90 контингент, 69 изуч. дисциплины) | ws4 §3 (AC2), DB_VUZ | ✅ `bookprovision.enroll/unenroll` + таблица `bp_enrollment` (студент 90 ↔ дисциплина 69), идемпотентно | **✅** | Per-student связь студент↔дисциплина; тест `test_bp_enrollment`. Δ2026-06-27. |
| 4.3 | DISC поле 83 ↔ VUZ поле 68 | **LinkDisc.gbl**: двунаправленная синхронизация дисциплина↔контингент | ws4 §3, DB_VUZ | ✅ `sync_contingent_from_enrollments` — контингент дисциплины ← фактические записи (source='rdr'), дисциплина↔контингент | **✅** | Тест `test_bp_enrollment::sync_contingent_checks`. Δ2026-06-27. |
...
| 5.3 | КСУ «Пополнение записи» → авто-поля 17/18/19, 44–49, 145–158 | Авто-распределение партии по разделам/типам/языкам | ws2 §4 (AC2), recon #CMPL-05 | ✅ `access/ksu_auto.py` (`distribute` titles(17)/by_section(44-49)/by_type/by_language/printed, Σ-инвариант) разведён роутом `POST /api/ksu/distribute` (staff); тест `test_ksu_auto` 38/0 + `test_domain_routes` | **✅** | Авто-распределение разведено (наша модель; точный Мастер-алгоритм recon не раскрыл). Δ2026-06-27. |
...
| # | Источник (поле/запись) → цель | Триггер / семантика | Дизайн? (§) | Код? (модуль / «нет») | Статус | Примечание |
|---|---|---|---|---|---|---|
| 7.1 | IBIS поля **951/953/955** → ПТ-хранилище (TEXTS/ПБД) | 951 внешний URL/файл, 953 встроенный двоичный, 955 ПТ-метаданные (`^A`файл, `^N`страниц) | ARCHITECTURE §3 (Файлы/Хранилище, Сканирование→951/953/955), FINDINGS_09 | ✅ `access/dam.py` разведён роутами `POST /api/dam/attach` (staff) + `GET /api/dam?db=&mfn=` (951/953/955↔запись, `rights_template`=955^B); тест `test_dam` 30/0 + `test_domain_routes` | **✅** | Связь запись↔бинарь разведена; реальное хранилище (S3/MinIO) — деплой. Δ2026-06-27. |
...
| 7.6 | VKR (ВКР) загрузка → IBIS + TEXTS | Загрузка ВКР студентом (`reg.frm`), конвертация `fst_rec.fst`, антиплагиат (215^W %) | FINDINGS_09, DB_CATALOG_VARIANTS | ✅ `access/vkr.py` разведён роутами `POST /api/vkr` (студент) + `GET /api/vkr` + `POST /api/vkr/review` (staff): submit/антиплагиат 215^W/review; тест `test_vkr` 31/0 + `test_domain_routes` | **✅** | ВКР-поток разведён; файл связывается через DAM (7.1). Δ2026-06-27. |
...
| 8.3 | IMAGE/VKR поле **903** ↔ IBIS `I=` | Образный/ВКР-каталог ссылается на запись ЭК по шифру (903 из 952^b) | DB_CATALOG_VARIANTS | ✅ `vkr.link_catalog(db,mfn,shifr)` — бэкреф ВКР→ЭК по шифру 903; ВКР-роуты разведены (`/api/vkr*`) | **✅** | Бэкреф ВКР→каталог разведён; обобщение на IMAGE — отдельно. Δ2026-06-27. |
...
| 9.2 | Номер журнала (NJ) ↔ журнал (J) — 461/46 | Иерархия журнал↔номер (461 общая часть, 46 доп. серийные) | CAPABILITY_MAP §4 | ✅ `access/serials.py` (register journal + add_issue → иерархия 461/46) разведён роутами `POST /api/serials\|/issue` + `GET /api/serials/search`; `test_serials` 21/0 + `test_domain_routes` | **✅** | Журнал↔номер разведён. Δ2026-06-27. |
| 9.3 | Многотомник: том ↔ сводная — 481/963/SPEC | Связь том↔многотомное издание | CAPABILITY_MAP §4 | ✅ `serials` kind=multivolume + add_issue(volume) → том↔сводная (481/963); тот же роут-набор `/api/serials/*` | **✅** | Многотомник разведён. Δ2026-06-27. |
...
| 10.3 | RDR студент 90/69 → VUZ контингент | Студент видит свои дисциплины/списки литературы (ЛК) | ws4 §6 (AC2), SPEC_reader_jirbis | ✅ `bookprovision.student_disciplines(student)` — студент видит свои дисциплины (ЛК) по записям `bp_enrollment` | **✅** | ЛК студента; тест `test_bp_enrollment::student_disciplines_checks`. Δ2026-06-27. |
...
|---|---|---|---|
| ✅ разведено (код+тесты) | ~53 | ~8 | **~61 (~97%)** |
| 🟡 спроектировано-не-разведено | ~2 | 0 | ~2 |
| ❌ отсутствует | ~0 | 0 | ~0 |
| **Итого рёбер** | 55 | 8 | **~63** |

> Δ2026-06-27 (последний проход ❌→✅): **1.3/1.4/1.5 ToCat** (938/901/VD=DEL) — уже были в `acquisition.to_catalog`+`test_tocat` (карта отставала, реклассификация) · **4.2/4.3/10.3 студент-синхро** — закрыты новым кодом: `bookprovision` таблица `bp_enrollment` + `enroll`/`student_disciplines`/`sync_contingent_from_enrollments` (`test_bp_enrollment` 17/0; регрессий нет — bookprovision 62/0, ko 71/0). **❌ обнулён.** Остаток 🟡 (~2): **11.4** seed→лимиты (мягкий — `category_limits` уже падает в `_DEFAULT`) · **11.1** инверсия RDR/CMPL/VUZ (каталог индексирован, остальные БД частично). То, что зависит от боевого дампа `jirbis2`/железа станций — вне карты рёбер (треки cutover/устройств).

> Δ2026-06-27 (РАЗВОДКА батч-2 в роуты `core.Api` — 6 рёбер 🟡→✅): **7.6/8.3** ВКР `/api/vkr*` · **5.3** КСУ `/api/ksu/distribute` · **9.2/9.3** периодика `/api/serials*` · **7.1** DAM `/api/dam*`. Все с route-тестами через настоящий `route()` — `tests/test_domain_routes.py` **48/0** (было 28); регрессий нет (circ_routes 80 · engine_routes 58). ✅ 52→58 (~92%), 🟡 7→1. Остаток ❌ (4): ToCat-остаток 1.3/1.4/1.5 (нужен `acquisition.py`) · студент-синхро 4.2/4.3/10.3 (нужен `bookprovision.py`) — общие файлы, развожу последовательно сам.
