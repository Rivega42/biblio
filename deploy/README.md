...
## Публичный демо-стенд (реестр росПО, #226)

Для проверки работающего экземпляра экспертами Минцифры (Единый реестр росПО)
есть **overlay-профиль демо** поверх этого же стека — own-store, **без живого
ИРБИС**, **без реальных ПДн**, помечен как ДЕМО, read-mostly, со сбросом по
расписанию и ротацией логов:

```sh
cd deploy
cp .env.demo.example .env     # окружение демо (секреты — СМЕНИТЬ; .env в gitignore)
sh proxy/gen-cert.sh          # на проде — реальный TLS, не самоподписанный
docker compose -f docker-compose.yml -f docker-compose.demo.yml up -d --build
sh demo/seed_demo.sh          # провижининг арендатора + демо-каталог + демо-читатели
sh demo/reset_demo.sh --snapshot   # зафиксировать эталон для периодического сброса
```

Полный гайд (требования к серверу **в РФ**, выдача демодоступа по заявке #226,
сброс/ретенция, соответствие 152-ФЗ для демо-данных) —
[`docs/deploy/DEMO_STAND.md`](../docs/deploy/DEMO_STAND.md).

## Развёртывание в контуре библиотеки (пилот)

📘 **Полный пошаговый гайд развёртывания в контуре библиотеки (СПб ГТБ)** —
[`docs/deploy/DEPLOYMENT.md`](../docs/deploy/DEPLOYMENT.md): системные требования,
быстрый старт, TLS (внутренний УЦ / Let's Encrypt / самоподпись), подключение к их
живому ИРБИС **или** own-store с миграцией, персистентность + бэкап/restore, постура
152-ФЗ, обновление/откат, чек-лист приёмки и траблшутинг, и что остаётся человеку
(реальный сервер, домен, сертификат, сетевой доступ к их ИРБИС).

См. также [`docs/design/PILOT_READINESS.md`](../docs/design/PILOT_READINESS.md) —
конкретный go-live чек-лист по DoD (реальные секреты, реальный TLS-сертификат,
провижининг первого арендатора, расписание бэкапов + проверенный restore, доступ к
мониторингу/логам, миграция данных, план отката), честный в том, что готово к пилоту,
а что нет (нет HA, нет PITR, нет сертифицированных СКЗИ, мост к живому ИРБИС
опционален).
...
| `docker-compose.demo.yml` | Overlay публичного демо-стенда (own-store, без ИРБИС/ПДн, ротация логов, метки демо) — реестр росПО #226 |
| `demo/seed_demo.sh` | Наполнение демо: провижининг арендатора + демо-каталог + демо-читатели (вызывает штатные механизмы) |
| `demo/seed_demo_catalog.py` | ~10 показательных записей в own-store каталога через `CatalogStore.save()` (без ИРБИС, без ПДн) |
| `demo/reset_demo.sh` | Периодический сброс демо к эталонному снимку (golden-дамп + пере-сид каталога) |
| `.env.example` | Все env-переменные с безопасными dev-дефолтами + метки CHANGE IN PROD |
| `.env.demo.example` | Окружение профиля демо-стенда (`APP_ENV=demo`, параметры сида/сброса, без секретов) |
